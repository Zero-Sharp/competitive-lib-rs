          ...
          U: Debug + PartialOrd + Copy + Zero + Add + AddAssign + Sub + SubAssign + Signed,
{
    ...
    for from in 0..size {
        for (to, (cap, cost)) in graph.iter(from) {
    ...
    for from in 0..size {
        for (to, (_, cost)) in graph.iter(from) {
...
U: Debug + PartialOrd + Copy + Zero + Add + AddAssign + Sub + SubAssign + Signed,
{
    let mut scanned = vec![false; g_f_pot.len()];
    scanned[s] = true;
    let mut stack = vec![s];
    while let Some(from) = stack.pop() {
        for (to, _) in g_f_pot.iter(from) {
            if !scanned[to] {
                if b[to] < U::zero() {
                    return Some(to);
                }
...
fn augment_along_pass<A,B,C,D,U>(
    graph: &A,
...
fn update_pot_and_g_f_pot<A,B,C,D,U>(
    graph: &A,
    vec: &Vec<(U, usize)>,
    pot: &mut Vec<U>,
    g_f_pot: &mut C,
...
U: Debug + PartialOrd + Copy + Zero + Add + AddAssign + Sub + SubAssign + Signed,
{
    ...
    let mut ret: C = Graph::new(graph.len());
    for from in 0..graph.len() {
        for (to, _) in g_f_cap.iter(from) {
            if graph.is_edge(from,to) {
